            ...
            <div class="card">
                <div class="card-body">
                    <div class="flex items-center">
                        <h6 class="text-15 grow">{{ __('messages.holidays') }} {{ date('Y') }}</h6>
                        <div class="shrink-0">
                            ...
                            @foreach ($holidayList as $key => $value)
                                <tr>
                                    ...
                                    <td class="px-3.5 py-2.5 border border-slate-200 dark:border-zink-500">
                                        {{ ++$key }}</td>
                                    <td class="px-3.5 py-2.5 border border-slate-200 dark:border-zink-500">
                                        {{ Carbon\Carbon::parse($value->holiday_date)->format('l') }}</td>
                                    <td id="holiday_date" hidden
                                        class="px-3.5 py-2.5 border border-slate-200 dark:border-zink-500">
                                        {{ $value->holiday_date }}</td>
                                    <td class="px-3.5 py-2.5 border border-slate-200 dark:border-zink-500">
                                        {{ Carbon\Carbon::parse($value->holiday_date)->format('d M') }}</td>
                                    <td id="holiday_name"
                                        class="px-3.5 py-2.5 border border-slate-200 dark:border-zink-500">
                                        {{ $value->holiday_name }}</td>
                                    <td id="holiday_type"
                                        class="px-3.5 py-2.5 border border-slate-200 dark:border-zink-500">
                                        {{ $value->holiday_type }}</td>
                                    <td class="px-3.5 py-2.5 border border-slate-200 dark:border-zink-500">
                                        <div class="flex gap-2 justify-center">
                                            ...
                                            <a href="#!" data-modal-target="deleteModal" id="deleteHoliday"
                                                class="flex items-center justify-center transition-all duration-200 ease-linear rounded-md size-8 bg-slate-100 text-slate-500 hover:text-red-500 hover:bg-red-100 dark:bg-zink-600 dark:text-zink-200 dark:hover:text-red-500 dark:hover:bg-red-500/20"><i
                ...
                <form action="{{ route('hr/holidays/save') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 gap-4 xl:grid-cols-12">
                        <div class="xl:col-span-12">
                            ...
                            <select name="holiday_type"
                                ...
                                <option value="Restricted Holiday"
                                    {{ old('holiday_type') == 'Restricted Holiday' ? 'selected' : '' }}>
                                    {{ __('messages.restricted_holiday') }}</option>
                                <option value="Gazetted Holiday"
                                    {{ old('holiday_type') == 'Gazetted Holiday' ? 'selected' : '' }}>
                                    {{ __('messages.gazetted_holiday') }}</option>
                                <option value="Observance" {{ old('holiday_type') == 'Observance' ? 'selected' : '' }}>
                                    {{ __('messages.observance') }}</option>
                                <option value="Season" {{ old('holiday_type') == 'Season' ? 'selected' : '' }}>
                                    {{ __('messages.season') }}</option>
                            </select>
                            @error('holiday_type')
                        ...
                        <div class="xl:col-span-12">
                            ...
                            <input type="text" name="holiday_name" id="holidayInput"
                        ...
                        <div class="xl:col-span-12">
                            <label for="holidayDateInput"
                                class="inline-block mb-2 text-base font-medium">{{ __('messages.date') }}</label>
                            <input type="text" name="holiday_date" id="holidayDateInput"
                                class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200 @error('holiday_date') is-invalid @enderror "
                                placeholder="{{ __('messages.select_date') }}" data-provider="flatpickr"
                                data-date-format="d M, Y" value="{{ old('holiday_date') }}">
                            @error('holiday_date')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                    ...
                    <div class="grid grid-cols-1 gap-4 xl:grid-cols-12">
                        ...
                        <div class="xl:col-span-12">
                            <label for="holiday_date"
                                class="inline-block mb-2 text-base font-medium">{{ __('messages.date') }}</label>
                            <input type="text" name="holiday_date" id="e_holiday_date"
                                class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200 @error('holiday_date') is-invalid @enderror "
                                placeholder="{{ __('messages.select_date') }}" data-provider="flatpickr"
                                data-date-format="d M, Y" value="{{ old('holiday_date') }}">
                            @error('holiday_date')
