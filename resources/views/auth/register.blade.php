    ...
    <div class="mb-0 w-screen lg:w-[500px] card shadow-lg border-none shadow-slate-100 relative">
        <div class="!px-10 !py-12 card-body">
            ...
            <div class="mt-8 text-center">
                <h4 class="mb-1 text-custom-500 dark:text-custom-500">{{ __('messages.create_free_account') }}</h4>
                <p class="text-slate-500 dark:text-zink-200">{{ __('messages.get_free_account') }}</p>
            </div>
            ...
            <form action="{{ route('register') }}" class="mt-10" method="POST">
                ...
                <div class="mb-3">
                    <label for="username-field"
                        class="inline-block mb-2 text-base font-medium">{{ __('messages.name') }}</label>
                ...
                <div class="mb-3">
                    <label for="password"
                        class="inline-block mb-2 text-base font-medium">{{ __('messages.password') }}</label>
                ...
                <div class="mb-3">
                    <label for="password"
                        class="inline-block mb-2 text-base font-medium">{{ __('messages.password_confirmation') }}</label>
                    <input type="password" name="password_confirmation" id="password_confirmation"
                        class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                        placeholder="{{ __('messages.enter_password_confirmation') }}">
                </div>
                <p class="italic text-15 text-slate-500 dark:text-zink-200">{{ __('messages.by_registering') }} <a
                        href="#!" class="underline">{{ __('messages.terms_of_use') }}</a></p>
                <div class="mt-10">
                    <button type="submit"
                        class="w-full text-white transition-all duration-200 ease-linear btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">{{ __('messages.sign_up') }}</button>
                </div>
                <div
                    ...
                    <h5
                        ...
                        {{ __('messages.create_account_with') }}</h5>
                </div>
                <div class="flex flex-wrap justify-center gap-2">
                    <button type="button" title="{{ __('messages.facebook') }}"
                    ...
                    <button type="button" title="{{ __('messages.google') }}"
                    ...
                    <button type="button" title="{{ __('messages.twitter') }}"
                    ...
                    <button type="button" title="{{ __('messages.github') }}"
                ...
                <div class="mt-10 text-center">
                    <p class="mb-0 text-slate-500 dark:text-zink-200">{{ __('messages.already_have_account') }}
                        <a href="{{ route('login') }}"
                            class="font-semibold underline transition-all duration-150 ease-linear text-slate-500 dark:text-zink-200 hover:text-custom-500 dark:hover:text-custom-500">{{ __('messages.sign_in') }}</a>
                    </p>
                </div>
            </form>
        </div>
