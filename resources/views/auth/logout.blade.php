    ...
    <div class="mb-0 w-screen lg:w-[500px] card shadow-lg border-none shadow-slate-100 relative">
        <div class="!px-10 !py-12 card-body">
            ...
            <div class="mt-8 text-center">
                ...
                <h4 class="mb-2 text-custom-500 dark:text-custom-500">{{ __('messages.you_are_logged_out') }}</h4>
                <p class="mb-8 text-slate-500 dark:text-zink-200">{{ __('messages.thank_you_message') }}</p>
            </div>
            <a href="{{ route('login') }}"
                class="w-full text-white transition-all duration-200 ease-linear btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">{{ __('messages.sign_in') }}</a>
        </div>
    </div>
@endsection
